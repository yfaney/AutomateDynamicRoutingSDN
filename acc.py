...
import subprocess, httplib, urllib2, json, time, requests
from collections import OrderedDict

MAX_BPS = 100 * 1024 * 1024
URL_GET_LINKS = "http://localhost:8080/wm/topology/links/json"
URL_SET_COSTS = "http://localhost:8080/wm/ccbalancer/topocosts/json"
...
def set_cost(src_sw, src_port, dst_sw, dst_port, cost):
	...
	r = requests.post(URL_SET_COSTS, data=json.dumps(OrderedDict(data)), headers=headers)
	return
...
def get_links():
	j = urllib2.urlopen(URL_GET_LINKS)
	...
	for j_obj in j_obj_list:
		...
		list.append([src_sw,src_port,dst_sw,dst_port,0])
...
while True:
	...
	for link in links:
		...
		cost = get_cost(thp[0], thp[1])
		if link[4] >= cost*1.05 or link[4] <= cost*0.95:
			link[4] = cost
			costJson = costJson + get_cost_json(link[0], link[1], link[2], link[3], link[4]) + ","
	...
	headers = {'Content-type':'application/json', 'Accept':'topocosts/json'}
	print costJson
	r = requests.post(URL_SET_COSTS, data=costJson, headers=headers)
	print r.text
	print "Cost set: " + str(links)
	time.sleep(5)

#print parse_switch_if("s5-eth0") #For Testing
